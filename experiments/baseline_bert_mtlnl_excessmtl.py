...
from src.trainer.bert_noise_aware_trainer import BertNoiseAwareTrainer


def main():
    parser = argparse.ArgumentParser(
        description="BERT MTL-NL / ExcessMTL baselines")
    parser.add_argument("--model", default="bert-base-uncased")
    parser.add_argument("--model_name", default="baseline_bert_noise_aware")
    parser.add_argument("--tasks", nargs="+", default=["MRPC", "RTE", "COLA"])
    parser.add_argument("--method", choices=["mtlnl", "excessmtl"],
                        required=True)
    ...
    parser.add_argument("--mtlnl_anchor_frac", type=float, default=0.02,
                        help="Anchor set size as fraction of train per (task, class).")
    args = parser.parse_args()
    if args.sst2_ratio is not None:
        override_train_sample_ratio(GlueTask.SST2, args.sst2_ratio)
    set_seed(args.noise_seed)

    trainer = BertNoiseAwareTrainer(
        ...
        mtlnl_anchor_frac=args.mtlnl_anchor_frac,
    )
    trainer.train(
